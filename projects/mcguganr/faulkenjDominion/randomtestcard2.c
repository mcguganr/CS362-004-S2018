...
int countSmithyAfter(struct gameState* G){
	int i, num = 0;
	for(i=1; i<(G->handCount[0] - 2); i++){
		if(G->hand[0][i] == smithy) num++;
	}
	return num;
}

int countSmithyBefore(struct gameState* G){
	int i, num = 0;
...
int main() {
	...
	while(numTests < 1000000){
		...
		for(j=0; j<numPlayers; j++){
			...
			G.hand[j][0] = smithy;//First card is great_hall
		...
		//Play the card
		if(cardEffect(smithy, choice1, choice2, choice3, &testG, handpos, &bonus)){
			printf("Failed to play card\n");
			numFails++;
		}else{
			//check all card counts to make sure things are as expected
			if((G.handCount[0] != 0) && assert(testG.handCount[thisPlayer] == (G.handCount[thisPlayer] + 2) )){
				printf("hand count = %d, expected = %d\n", testG.handCount[thisPlayer], G.handCount[thisPlayer] + 2);
				numFails++;
			} else if( (G.handCount[0] != 0) && assert(countSmithyAfter(&testG) == (countSmithyBefore(&G) - 1) )){
				printf("card not actually drawn %d %d\n", countSmithyAfter(&testG), (countSmithyBefore(&G) - 1));
				numFails++;
			}

			if(assert((testG.deckCount[thisPlayer] + testG.discardCount[thisPlayer] + testG.handCount[thisPlayer] + testG.playedCardCount) == 
				(G.deckCount[thisPlayer] + G.handCount[thisPlayer] + G.discardCount[thisPlayer]) + G.playedCardCount) - 1){
